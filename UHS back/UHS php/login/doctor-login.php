...
<i class="fa fa-user-md align-self-center border-2 border-danger-subtle border p-3 rounded-5 bg-body-tertiary shadow-lg mb-4 mt-2" style=" font-size: 60px; color: #080276;"></i>
...
$getTable_data="SELECT `d_uid`,`email`,`password` FROM `doctor` WHERE `email`='$email'";

$getData=mysqli_query($connection,$getTable_data);

    //VERIFYING THE EMAIL ID & PASSWORD

    $docData = mysqli_fetch_array($getData);
    if(!password_verify($passWord,$docData['password']))
    {
        echo'<span class="text-center text-danger mt-4" style="font-size: 20px; font-weight: 550;">Wrong email or password</span> ';
    }
    else{
        $_SESSION['duid'] = $docData['d_uid'];
        header('location:../newAdmin/doctor-home.php');
        exit();
    }
